	...
	public:
		...
		void LifeCycle()
		{
			...
			while (m_window.isOpen())
			{
				...
				for (auto it = m_ship.GetBullets().begin(); it != m_ship.GetBullets().end();)
				{
					if (!it->Available())
					{
						it = m_ship.GetBullets().erase(it);
					}
					else
					{
						it++;
